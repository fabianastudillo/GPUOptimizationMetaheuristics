...
def EvaluacionParticulas(swarm, PN):
    ...
    # TOPOLOGIA PARA UN SISTEMA GARVER
        ...
        for j in swarm[:,i]:
            if j != 0:
                for k in range (np.int(j)):
                    exec('l{} = pp.create_line(net, vias[0][cont],vias[1][cont],parallel=4,max_loading_percent = 50, length_km = 1., std_type = "149-AL1/24-ST1A 110.0")'.format(cont))
            cont = cont +1; 
        ...
        pp.create_load(net, bus4, p_mw = 180)  
        ...
        pp.runopp(net, numba = True, verbose= False, calculate_voltage_angles = False, check_connectivity=False)     
        #Penalizacion
...
while STOP == 0:     
    ...
    for i in range(0,PN):
        ...
        R1 = cp.asarray(R1)
        R2 = cp.asarray(R2)
        bestpos = cp.asarray(bestpos)
        swarm2 = cp.asarray(swarm2)
        vel = cp.asarray(vel)
       ...
       # G = chi*(vel[:,i] + c1*R1*(bestpos[:,i]-swarm2[:,i])+c2*R2*(bestpos[:,ifxopt]-swarm2[:,i]))
        start = timer()
        G = chi*(vel[:,i] + cp.multiply(R1,(bestpos[:,i]-swarm2[:,i]))*2.05+cp.multiply(R2,(bestpos[:,ifxopt]-swarm2[:,i]))*2.05)
        cp.cuda.Stream.null.synchronize()
        
        #Actualizacion del valor de velocidad
        TEPtime = timer() - start
        TEPtotalgpu = TEPtotalgpu+TEPtime
        print(TEPtotalgpu)
        vel = cp.asnumpy(vel)
        G = cp.asnumpy(G)
        vel[:,i] = G;
    ...
    print(xopt)
print("tiempo PSO en CPU %f seconds." % TEPtotalgpu)